...
const placeOrder = async (req, res) => {
  try {
    ...
    if (!payment_option || !selectedAddress) {
      console.log(
        "validating cartId payemtoption selectedaddress",
        payment_option,
        selectedAddress
      );
    ...
    switch (payment_option) {
      ...
      default:
        return res
          .status(400)
          .json({ success: false, message: "Invalid payment option" });
    }
    ...
    if (blockedProducts.length > 0) {
      return res.status(400).json({
        success: false,
        message:
          "Some products in your cart are blocked and cannot be purchased.",
    ...
    let subtotal = userCart.items.reduce(
    ...
    if (paymentMethod === "Wallet") {
      try {
        ...
        return res
          .status(500)
          .json({ success: false, message: "Wallet payment processing error" });
      }
    }
